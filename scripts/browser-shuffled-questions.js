// Browser Console Script to Add Shuffled Questions
...
// Function to add all shuffled questions
async function addShuffledQuestions() {
  ...
  for (let i = 0; i < shuffledQuestions.length; i++) {
    ...
    try {
      ...
      if (response.ok) {
        successCount++;
        console.log(`✅ Added question ${i + 1}/${shuffledQuestions.length}: [${question.category}] ${question.text.substring(0, 50)}...`);
      } else {
        errorCount++;
  ...
  return { successCount, errorCount, shuffledQuestions };
}
    ...
    if (response.ok) {
      ...
      if (questions.length === allQuestions.length) {
        ...
        questions.slice(0, 10).forEach((q, index) => {
          console.log(`${index + 1}. [${q.category}] ${q.text.substring(0, 60)}...`);
        });
        console.log('');
      ...
      console.log('❌ Failed to verify questions');
      return false;
    }
  } catch (error) {
...
async function updateDatabaseWithShuffledQuestions() {
  try {
    // Step 1: Clear existing questions
    await clearAllQuestions();
    
    // Step 2: Add all shuffled questions
    const { successCount, errorCount, shuffledQuestions } = await addShuffledQuestions();
    
    // Step 3: Summary
    console.log(`\n📊 Update Summary:`);
    console.log(`✅ Successfully added: ${successCount} questions`);
    console.log(`❌ Failed to add: ${errorCount} questions`);
    console.log(`📋 Total questions processed: ${shuffledQuestions.length}`);
    
    // Step 4: Verify
    const verified = await verifyQuestions();
    
    if (verified && successCount === allQuestions.length) {
      console.log('\n🎉 COMPLETE SUCCESS! All shuffled questions added and verified!');
    } else {
      console.log(`\n⚠️ WARNING: Expected ${allQuestions.length} questions, but only ${successCount} were added successfully`);
    }
...
// Run the update
updateDatabaseWithShuffledQuestions();
